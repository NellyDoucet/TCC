...
(function (window) {
  ...
  /* Classement : une liste d'elements a ranger chacun dans une categorie
     (ex. charge d'exploitation, produit financier...) via un menu deroulant. */
  function renderClassement(question, index, state) {
    var list = el('div', 'classement');
    var refs = [];
    question.items.forEach(function (item, itemIndex) {
      var row = el('label', 'classement__ligne');
      row.appendChild(el('span', 'classement__libelle', item.label));
      var select = document.createElement('select');
      select.className = 'classement__choix';
      select.id = 'q-' + question._uid + '-' + itemIndex;
      select.appendChild(el('option', null, 'Choisir…'));
      select.options[0].value = '';
      question.categories.forEach(function (cat, catIndex) {
        var opt = el('option', null, cat);
        opt.value = String(catIndex);
        select.appendChild(opt);
      });
      row.appendChild(select);
      list.appendChild(row);
      refs.push({ item: item, select: select, row: row });
    });
    state.classementRefs = refs;
    state.getValue = function () {
    ...
    state.setDisabled = function (disabled) {
      refs.forEach(function (ref) {
        ref.select.disabled = disabled;
      });
    };
    state.markState = function (allCorrect, results) {
      refs.forEach(function (ref, i) {
        var ok = results ? results[i] : null;
        ref.row.classList.toggle('is-correct', ok === true);
        ref.row.classList.toggle('is-incorrect', ok === false);
      });
    };
    return list;
  }

  function evaluateClassement(question, state) {
    var results = [];
    var errorMessages = [];
    state.classementRefs.forEach(function (ref) {
      var ok = ref.select.value !== '' && parseInt(ref.select.value, 10) === ref.item.answer;
      results.push(ok);
      if (!ok) {
        errorMessages.push(ref.item.label + ' → ' + question.categories[ref.item.answer]);
      }
    });
    return {
      allCorrect: results.every(function (r) { return r; }),
      results: results,
  ...
  function createExercise(container, data, options) {
    ...
    data.questions.forEach(function (question, index) {
      ...
      } else if (question.type === 'classement') {
        control = renderClassement(question, index, state);
      }
    ...
    function evaluateQuestion(state) {
      ...
      if (question.type === 'numeric') {
        ...
        var classResult = evaluateClassement(question, state);
        isCorrect = classResult.allCorrect;
        errorText = classResult.errorMessages.length
          ? 'À corriger : ' + classResult.errorMessages.join(' ; ') + '.' + (question.explanation ? ' ' + question.explanation : '')
          : '';
        state.markState(isCorrect, classResult.results);
      }
      ...
      state.feedbackHolder.appendChild(buildFeedback(isCorrect, errorText, (question.type === 'table' || question.type === 'classement') ? '' : correctionText));
      state.checked = true;
    ...
    resetBtn.addEventListener('click', function () {
      questionStates.forEach(function (state) {
        ...
        var selects = state.element.querySelectorAll('select');
        selects.forEach(function (select) {
          select.value = '';
        });
        var lignes = state.element.querySelectorAll('.classement__ligne');
        lignes.forEach(function (ligne) {
          ligne.classList.remove('is-correct', 'is-incorrect');
        });
      });
